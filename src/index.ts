...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Noaa SPC');
}
...
const tools: McpToolExport['tools'] = [
  ...
  { name: 'mesoscale_discussions', description: 'Recent mesoscale discussions list (as text page).', inputSchema: { type: 'object', properties: {} } },
  { name: 'storm_reports', description: 'Fetch NOAA SPC preliminary storm reports CSV for a given date (YYMMDD, default today UTC) filtered by kind: tornado, wind, hail, or filtered (combined). Returns raw CSV with time, location, magnitude, and remarks.', inputSchema: { type: 'object', properties: { date: { type: 'string', description: 'YYMMDD (default today UTC)' }, kind: { type: 'string', description: 'tornado | wind | hail | filtered (default)' } } } },
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    ...
    case 'mesoscale_discussions': {
      const res = await pwFetch(`${BASE}/products/md/`, { headers: { 'User-Agent': UA } });
      if (!res.ok) throw await httpError(res, 'SPC MD page');
    ...
    case 'storm_reports': {
      ...
      if (!res.ok) throw await httpError(res, 'SPC storm reports');
      return { format: 'csv', body: await res.text() };
    }
    case 'watches_active': {
      const res = await pwFetch(`${BASE}/products/watch/`, { headers: { 'User-Agent': UA } });
      if (!res.ok) throw await httpError(res, 'SPC watches');
      return { format: 'html', body: (await res.text()).slice(0, 30000) };
    }
    default:
...
async function spcJson(url: string): Promise<unknown> {
  ...
  if (!res.ok) throw await httpError(res, 'SPC');
  return res.json();
}
